...
- **grey (neutral)** only where an outcome is genuinely unobservable — a REPL round whose shell reports no per-round exit status (when the persistent shell does report it, settled rounds take done/error like any execution), and a non-final command inside a single multi-command call (`echo a; false; echo b`) yields no per-command status: the harness observes one exit code per call, and a single call running several commands with per-command status is not a current capability anywhere. Both are grey. The skeleton never parses a Traceback to invent a red lamp for an outcome it cannot observe. Per-Turn lamps therefore appear where the harness observes the turn's own outcome — a single-command call, or one round of an interactive session. Capturing per-command status (an executor change) is deferred with no field in the type — when built, it lands as a compile-breaking extension alongside its consumer, per the same discipline as render kinds and recursion — and would upgrade a multi-command call's intermediate turns from grey.

`warn` (amber) is the one new state relative to the current three-state `StateDot` use; the token already exists ([StateDot.module.css](../../../../packages/client/ui-primitives/src/StateDot.module.css)). Signal attribution uses only the harness's own signals, never a guess at who sent a signal, because the OS does not report the sender. The amber inputs split by channel. A call that vanishes mid-flow gets a client-synthesized `error.code: 'interrupted'` result node — the `stopped` row state's source today, a pure function of the event stream that runs on the replay path too (history-fold.ts) — and the lamp maps that to amber. `timedOut` lives in the bash result value on the success path, which a presenter never sees (`presentationMeta` runs only there), so it must ride the new bash `presentationMeta`. A *settled* abort is different: the harness persists `code: 'ABORTED'` for a dispatch-level abort, and a bash mid-run abort persists a plain `Error('command aborted')` with no distinguishing code — neither is attributable as amber by a replayed client today, so a settled abort renders red. Making it amber requires the bash abort path to persist a distinguishable attribution — a deferred executor change like per-command capture — and this note does not promise amber for settled aborts until that lands.
...
The scrollbar reuses the repo's themed-scrollbar mechanism rather than a self-drawn overlay. Native bars stay; they are skinned through ui-theme's `--dsh-scrollbar-thumb`/`-hover` indirection pair (both the WebKit pseudo-element and Firefox `scrollbar-color` paths), and a segment reveals its bar only while scrolling or hovered by rebinding that pair the way [the pointer-revealed-scrollbars note](../../implemented/feature/2026-08-04-pointer-revealed-sidebar-scrollbars.md) does for the sidebar — to the l2 pair when active, to `transparent` at rest — with a short linger tail. The reserved-gutter reservation ([2026-07-28-themed-scrollbars-and-reserved-gutter.md](../../implemented/bug-fix/2026-07-28-themed-scrollbars-and-reserved-gutter.md)) keeps the body line from shifting when the thumb appears, so `scrollbar-width: none` (which would collapse that reservation) is not used. This is deliberately **not** the self-drawn overlay the pointer-revealed note weighed and rejected — that path costs hit-testing, drag, wheel, momentum, and both palettes' hover states for a cosmetic gain; reusing the token indirection gets the reveal for free and inherits the elevated-surface rebind contract. Long single lines overflow horizontally the same way, and the horizontal bar follows the same reveal.
...
| bash (N cmds) | one prompt line per command | the call's merged output | one lamp for the whole call today (the harness observes a single exit per call); per-command Turns with per-command lamps need the deferred executor change (§lamp) |
| bash (REPL) | one Turn per stdin round, `>>>` prompt | round output | active round blue; settled rounds done/error where the shell reports per-round exit status, grey where it does not |
...
`Segment.render` is a **closed** tagged union of **render kinds**. It holds only the implemented kinds today (`prompt` / `text` / `lines` / `diff`); the envisioned future vocabulary (`kv` / `link` / `json` / `table` / `image` / `notice`) is *not* in the union yet — each is added, with its renderer, when a consumer needs it, so a segment's payload is described by data rather than bespoke per-tool components. This is what lets custom tools reach the skeleton in three tiers:
...
3. **bash** converted: a single-command call renders as one Turn, and a new bash `presentationMeta` carries the structured single-command result (command, cwd, output, exit/signal/timeout). Per-command Turns for a joined multi-command call, and per-round Turns for a persistent interactive shell, are the deferred executor change (§lamp, §table) — not built in 1c; a multi-command call stays one Turn here. First real tool — the snapshot/e2e coverage that needs real tool data lands here.
