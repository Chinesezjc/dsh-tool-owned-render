...
- **The registration interface**: the per-view keyed toolview slot family (`conversation.chat.toolview` today; `…details.toolview` and PR 3's preview slot land with their render sites), all sharing one `kind`/`scope`/`owner` shape ([slots.ts:168-203](../../../../packages/client/ui-conversation/src/client/contract/slots.ts#L168)), plus a `registerToolView(ctx, key, component)` fan-out that registers one component against every declared toolview slot at once, and the props a registered component receives (the tool's call view and result view).
- **Built-in registrants**: one self-contained module per built-in tool (`bash`, `read`, `search`, `web`, `write`/`edit`, …), each registering its own React component that composes the primitives and does its own wire→props. `ui-conversation` shrinks to the chat chrome (messages, compaction, queue, input) and mounts the slots.

There is **no central render dispatch and no client-side render-kind union**. Per view, that view's keyed toolview slot *is* the dispatch: each tool call renders through the component registered under its tool name, or through the generic fallback when none is registered. The registry of registrants is shared across the per-view slots via `registerToolView`, so the chat stream, the details panel, and PR 3's preview panel all render a tool through the same registrant — no cross-plugin component import and no passing a `ReactNode` across the slot boundary, which the slot contract forbids. Changing `read`'s rendering edits the `read` registrant module — its component and its wire→props — which depends only on the package's primitives; it touches no central switch, no client render-kind union, and no other tool. Adding a new tool's rendering is a new registrant with zero central change. A third-party tool author depends on this one package and ships their own registrant.

This isolation is scoped to the **client render path** — the dispatch and the component. It does **not** remove the host-side view vocabulary: `presentCall`/`presentResult` still return the closed `ToolCallView`/`ToolResultView` unions in [presentation.ts](../../../../packages/core/tools/src/presentation.ts) (§data source), the one channel that survives replay. A tool whose data fits an existing view variant — or falls to `generic` — needs no central change; a tool needing a **genuinely new structured payload** across the host→client boundary extends that closed union, exactly as it does today. That boundary is deliberately preserved (it is what keeps model-visible text and the UI's structured view reconstructable from one execution), and it is orthogonal to who owns rendering.
...
The package exports one lamp-state function over what the harness can observe; a tool's registrant feeds it the result and hands the state to a `Segment` or `Group`. It replaces today's four independent status derivations. A tool may pass its own state, but the shared helper is what makes every tool agree. The helper checks in order — specific signals *before* the generic `isError` rules, because both a cancelled result and a failed terminal command carry an `isError` value the generic rules would mis-read:

- **cancelled or interrupted → amber (warn).** A dispatch-level abort persists `error.info.code` = `ABORTED` (or `ABORTED_BEFORE_DISPATCH`) on the result of *any* tool ([tools/index.ts:1180/1588/1602](../../../../packages/core/tools/src/index.ts#L1180)), terminal or not, so a cancelled read, web, or bash maps to amber; the client-synthesised `interrupted` code the `stopped` row state reads maps to amber too. Checked first, because a cancelled/interrupted result settles `isError: true` *and* carries the code — the generic error rule below would mis-map it to red.
